...
from utils.ast import ASTNode as ASTNode
from utils.ast import json_to_ast as json_to_ast
...
from utils.rich_sketch import RichSketchNode as RichSketchNode
from utils.definitions import node_type_to_sketch as node_type_to_sketch
from utils.rich_sketch import sketch_to_json as sketch_to_json
from utils.rich_sketch import json_to_sketch as json_to_sketch
...
class GenerateRichSketch:

    def __init__(self, program:ASTNode, type='hoc'):
        self._program = program
        self._type = type
        self._sketch_without_pruning = self.get_raw_sketch_from_program(program)
    ...
    def create_sketch_node(self, node_type, conditional=None, children=[], conditional_val=None, node_val = []):
        cfg_node = RichSketchNode(node_type, conditional, children, conditional_val=conditional_val, node_val=node_val)
        return cfg_node

    def get_raw_sketch_from_program(self, prog: ASTNode):

        if node_type_to_sketch[prog._type_enum] == 'A':
            # save the node-value in a list
            val_node = [prog._type]
            if prog.conditional() is not None:
                if prog.conditional() in ['2', '3', '4', '5', '6', '7', '8', '9']:
                    cfg = self.create_sketch_node(node_type_to_sketch[prog._type_enum], 'X',  children=[],
                                                  conditional_val=prog.conditional(), node_val=val_node)
                else:
                    cfg = self.create_sketch_node(node_type_to_sketch[prog._type_enum],
                                                  'bool_cond', children=[], conditional_val=prog.conditional(), node_val=val_node)
            else:
                cfg = self.create_sketch_node(node_type_to_sketch[prog._type_enum], None,  children=[], node_val=val_node)
        else:
            if prog.conditional() is not None:
                if prog.conditional() in ['2', '3', '4', '5', '6', '7', '8', '9']:
                    cfg = self.create_sketch_node(node_type_to_sketch[prog._type_enum], 'X', children=[],
                                                  conditional_val=prog.conditional())
                else:
    ...
    def prune_raw_sketch(self, sketch: RichSketchNode):
        # merge A_BLOCK nodes
            ...
            for i in range(len(queue)):
                ...
                for j in range(len(node._children) - 1):
                    if node._children[j]._type == 'A' \
                        ...
                        node_vals.extend(node._children[j + 1]._node_val) # save the list of action-vals
                        delete_indices.append(j + 1)
                        curr_iter_indices.append(j+1)
                        # print("Node vals:", node_vals, curr_iter_indices)
                        # node._children[delete_indices[0] - 1]._alphabet['block'].append(
                        #     node._children[j +1]._alphabet['block'][0])
                        if j+1 == len(node._children)-1: # last child of node: add the entries for this as well
                            if len(curr_iter_indices) > 0:
                                min_ele = min(curr_iter_indices)
                                vals_with_index.append([min_ele - 1, node_vals])
                            node_vals = []
                            curr_iter_indices = []
                            # print("Vals with index:", vals_with_index)


                    else:
                        if node._children[j]._type == 'A' or node._children[j+1]._type == 'A':
                            if len(curr_iter_indices) > 0:
                                min_ele = min(curr_iter_indices)
                                vals_with_index.append([min_ele-1, node_vals])
                            # print("Vals with index:", vals_with_index)
                ...
                # add the list values to the indices in vals_with_index
                for ele in vals_with_index:
                    if node._children[ele[0]]._type == "A": # double check the ID type
                        node._children[ele[0]]._node_val.extend(ele[1])
    ...
    def remove_action_nodes(self, sketch: RichSketchNode):
    ...
    def modify_sketch_tree_level_1(self, sketch: RichSketchNode):
    ...
    def modify_sketch_tree_level_2(self, sketch: RichSketchNode):
    ...
    def modify_sketch_tree_level_3(self, sketch: RichSketchNode):
    ...
    def modify_sketch_tree_level_4(self, sketch:RichSketchNode):
        if sketch._type == 'S;S':
            cfg = self.create_sketch_node('S', None, children=[])
            cfg._children = [copy.deepcopy(sketch)]
            sketch = cfg
    ...
    def modify_sketch_tree_level_5(self, sketch: RichSketchNode):
    ...
    def get_modified_sketch(self, sketch: RichSketchNode):
...
def get_hash_code_of_sketch(sketch: RichSketchNode):
