...
from code.step2_struct2code.smt_constraints_task3.delta_debugging_task3 import get_delta_debugged_validity
from code.utils.ast import ASTNode, json_to_ast, ast_to_json, get_size, tree_edit_distance, valid_ASTNode


DATA_INPUT_FOLDER = 'data/output/task-3/substructure-3/all-mutations/'
DATA_OUTPUT_FOLDER = 'data/output/task-3/substructure-3/'
TYPE = 'hoc'
CODE_BUCKETS = {
    "7": list(range(1,2)),
    "8": list(range(1,21)),
...
def wrapper_quick_gen():
    # obtain the code_set to run the full pipeline
    code_set, num_codes = get_all_mutation_codes()
    ...
    # temp-folder
    temp_folder_name = DATA_OUTPUT_FOLDER  + "temp_misc"
...
def get_all_task_code_pairs():
    final_task_code_set, num_codes = wrapper_full_task_gen()
