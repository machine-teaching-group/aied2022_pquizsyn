...
from code.step2_struct2code.sym_ast import AST, valid_prog, get_hash_code_of_symast
...
def run_delta_set_on_task(type:str, code:AST, start_karel_world, end_karel_world, temp_folder=DATA_TEMP):
    '''
...
DATA_TEMP = ''
def get_tasks_for_code(type:str, code:AST, temp_folder = DATA_TEMP, task_iters=40000):
    ...
    # cond-flag
    if code._n_if > 0 or code._n_while > 0 or code._n_else > 0 or code._n_repeat_until_goal > 0:
        cond_flag = True
    else:
        cond_flag = False
...
def remove_single_node_from_ast(root:AST):
    ...
    while len(queue):
        for i in range(len(queue)):
            node = queue.popleft()
            node_children = [child.name for child in node.children]
            if 'move' or 'turn_left' or 'turn_right' or 'put_marker' or 'pick_marker' in node_children:
                if len(node_children)>1:
                    ...
                    if len(indices) != 0:
                        for i in indices:
                            c = node.children.pop(i)
                            new_node = copy.deepcopy(root)
                            new_node._hash = get_hash_code_of_symast(new_node)
                            new_ast_nodes.append(new_node)
                            node.children.insert(i, c)
...
def get_delta_codes(code:AST):
    delta_set = list(set(remove_single_node_from_ast(code)))
    # remove the invalid nodes
    delta_set = [ele for ele in delta_set if valid_prog(ele)]
    delta_set = list(set(delta_set))

    return delta_set


def get_delta_debugged_validity(type:str, code:AST, start_task, end_task, temp_folder=DATA_TEMP):
    '''
