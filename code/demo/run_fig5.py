...
from code.step4_intervention.gen_output_code_task_image import gen_output_code_mcq_images, get_blank_token_in_json, parse_ast_blank, gen_output_mcq_script
...
def get_solution_code_symast_format():
    ...
    code = AST('run', [
        AST('repeat(8)', [
        AST('ifelse(bool_no_marker)', [
            AST('do', [AST('put_marker')]),
            AST('else', [
                AST('pick_marker')
            ])
        ]),
...
def get_student_attempt():
    ...
    stu_0 = ASTNode('run', None, [
        ...
        ASTNode('pick_marker'),
        ASTNode('move'), ASTNode('move'),
        ASTNode('put_marker'), ASTNode('move'),
        ASTNode('put_marker'), ASTNode('move'),
        ASTNode('put_marker'), ASTNode('move'),
    ])
...
def generate_input(inputfolder = 'data/demo/fig5/', inputfolder_temp = 'data/demo/fig5/temp_latex/'):
    task= get_task()
...
def get_sketch_hint(student_sketch, solution_sketch, type='karel'):
    sketch_hint = get_sketch_multihop_2(student_sketch, solution_sketch, type=type)
    return sketch_hint


def generate_all_images(inputfolder = 'data/demo/fig5/', inputfolder_temp = 'data/demo/fig5/temp_latex/'):
    ...
    os.system("pdflatex -interaction=nonstopmode -output-directory data/demo/fig5/temp_latex %s" % (input_path))
    output_path = inputfolder_temp + 'code_Cin_solution.pdf'
    shutil.copyfile(output_path, 'data/demo/fig5/code_Cin_solution.pdf')
    # os.system("convert -density 1200 -quality 100 data/demo/fig5/temp_latex/code_Cin_solution.pdf %s" % output_path)
    ...
    shutil.copyfile(output_path, 'data/demo/fig5/code_Cin_student.pdf')
    # os.system("convert -density 1200 -quality 100 data/demo/fig5/temp_latex/code_Cin_student.pdf %s" % output_path)
    ...
    shutil.copyfile(output_path, 'data/demo/fig5/code_Cquiz.pdf')
    # os.system("convert -density 1200 -quality 100 data/demo/fig5/temp_latex/code_Cquiz.pdf %s" % output_path)
    ...
    shutil.copyfile(output_path, 'data/demo/fig5/code_Cquiz_with_blank.pdf')
    # os.system("convert -density 1200 -quality 100 data/demo/fig5/temp_latex/code_Cquiz_with_blank.pdf %s" % output_path)

    ## MCQ image file
    mcq_script = gen_output_mcq_script('karel')
    ...
    shutil.copyfile(output_path, 'data/demo/fig5/quiz_options.pdf')
    # os.system("convert -density 1200 -quality 100 data/demo/fig5/temp_latex/quiz_options.pdf %s" % output_path)
    ...
    os.system("pdflatex -output-directory data/demo/fig5/temp_latex %s" % (input_path))
    output_path = inputfolder_temp + 'task_Tin.pdf'
    shutil.copyfile(output_path, 'data/demo/fig5/task_Tin.pdf')
    # os.system("convert -density 1200 -quality 100 data/demo/fig5/temp_latex/task_Tin.pdf %s" % output_path)
    ...
    shutil.copyfile(output_path, 'data/demo/fig5/task_Tquiz.pdf')
    # os.system("convert -density 1200 -quality 100 data/demo/fig5/temp_latex/task_Tquiz.pdf %s" % output_path)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run Demo for Code.org task: Karel-Opposite.')
    ...
    print("Obtaining minimal code from Squiz.... (this may take upto a minute!).....")
    ...
    total_codes = get_single_task_code_pair_from_minimal_code('karel', 4, mincode, max_blk_size,
                                                                    sol_code_symast,
                                                                    bucket_sample_size=1,
                                                                    delta_debugging_sample_size=1,
                                                                    output_folder=data_output, verbose=False)


    # generate the code with blank
    print("******************************************************* Stage 3: Generating Multi-Choice Question from (Tquiz, Cquiz)")
    with open('data/demo/fig5/code_Cquiz.json', 'r') as f:
        code_cquiz = json.load(f)
    code_with_blank, mcq_option = get_blank_token_in_json(code_cquiz, type='karel')
    with open('data/demo/fig5/code_Cquiz_with_blank.json', 'w') as f:
