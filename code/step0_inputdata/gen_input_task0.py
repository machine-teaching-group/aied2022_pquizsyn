...
def get_solution_code():
    ...
    :return: the solution code, and code struct of the task
    '''

    code = ASTNode('run', None, [
        ASTNode('repeat', '4', [ASTNode('move')]), ASTNode('turn_left'),
...
def get_solution_code_symast_format():
    ...
    '''
    code = AST('run', [
        AST('repeat(4)', [AST('move')]), AST('turn_left'),
        AST('repeat(5)', [AST('move')])
        ])
...
def get_student_attempts():
    ...
    stu_2 = ASTNode('run', None, [
        ASTNode('repeat', '5', [ASTNode('move')]), ASTNode('turn_right'),
        ASTNode('repeat', '5', [ASTNode('move')])]
                          )
...
def get_basic_data_for_task(folder='', save = True):
    ...
    if save:
        print("Saving the contents of task-0...")
...
if __name__ == "__main__":
    ...
    inputfolder = ''
    get_all_data_for_task(inputfolder)
